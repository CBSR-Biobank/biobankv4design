...
the \entitylink{Study}, to a paricipant visit. A participan visit is when
specimens are collected directly from the participant at a collection
...
A processing type should have \texttt{enabled} set to \texttt{true} if when
processing of specimens is taking place. However, throughout the lifetime of
the study, it may be decided to stop a processing type in favour of
another. In this case \texttt{enabled} is set to \texttt{false}.
