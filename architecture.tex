...
The client ports shown above are managed by client adapters. These client
adapters then interface to the rest of the system via \emph{Command Handlers}
and the \emph{Query Interface}. Figure \ref{fig:cqrs-architecture} shows a more
detailed view of the application. It is the architecture recommended by the
Axon Framwork\footnote{The figure and the material that follows is borrowed
  from the Axon Framwork Reference Guide \cite{AxonOnline}}.
...
used. This data layer typically returns read-only DTO objects containing query
results. The contents of these DTO's are typically driven by the needs of the
client adapters. In most cases, they map directly to a specific view in the UI
